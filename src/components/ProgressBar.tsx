...
const ProgressBar = ({ bgcolor, currentModulePercentage, setCurrentModulePercentage }: Props) => {
    const [currentFinishedModule, setCurrentFinishedModule] = useState<FinishedModule>();
    ...
    useEffect(() => {
        if (params) {
            ...
            if (currentModule && currentFinishedModule) {
                const completePercent = 100 / currentModule?.submenu.length * currentFinishedModule?.completedLessonId.length;
                if (!percentage) {
                    const obj = {
    ...
    }, [finishedModules, params, data, currentFinishedModule?.completedLessonId.length])

    useEffect(() => {
        const filter = percentage?.filter(item => item.moduleId === currentModule?.id)
        if (filter) {
